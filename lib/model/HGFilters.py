...
class HourGlass(nn.Module):
    ...
    def _generate_network(self, level):
        self.add_module('b1_' + str(level), ConvBlock(self.features, self.features, norm=self.norm, groupNormSize=self.groupSize))

        self.add_module('b2_' + str(level), ConvBlock(self.features, self.features, norm=self.norm, groupNormSize=self.groupSize))

        if level > 1:
            self._generate_network(level - 1)
        else:
            self.add_module('b2_plus_' + str(level), ConvBlock(self.features, self.features, norm=self.norm, groupNormSize=self.groupSize))

        self.add_module('b3_' + str(level), ConvBlock(self.features, self.features, norm=self.norm, groupNormSize=self.groupSize))
...
class HGFilter(nn.Module):
    def __init__(self, opt):
        ...
        if not self.opt.skip_downsample:
            ...
            elif self.opt.hg_down == 'ave_pool' or self.down_type == 'no_down':
                self.conv2 = ConvBlock(64, 128, self.opt.norm)
            else:
        ...
        for hg_module in range(self.num_modules):
            self.add_module('m' + str(hg_module), HourGlass(1, opt.num_hourglass, self.opt.hourglass_dim_internal, self.opt.norm, groupNormSize))

            self.add_module('top_m_' + str(hg_module), ConvBlock(self.opt.hourglass_dim_internal, self.opt.hourglass_dim_internal, self.opt.norm, groupNormSize))
            self.add_module('conv_last' + str(hg_module), nn.Conv2d(self.opt.hourglass_dim_internal, self.opt.hourglass_dim_internal, kernel_size=1, stride=1, padding=0))
            if self.opt.norm == 'batch':
                self.add_module('bn_end' + str(hg_module), nn.BatchNorm2d(self.opt.hourglass_dim_internal))
            elif self.opt.norm == 'group':
                self.add_module('bn_end' + str(hg_module), nn.GroupNorm(groupNormSize, self.opt.hourglass_dim_internal))

            self.add_module('l' + str(hg_module), nn.Conv2d(self.opt.hourglass_dim_internal,
